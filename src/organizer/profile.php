...
if (isset($_POST["update"])) {
    ...
    if (empty($name) || empty($phone) || empty($email) || empty($address) || empty($website) || empty($about) || empty($pwd)) {
...
<body>
     ...
     <aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full border-r sm:translate-x-0 bg-primary border-gray-700" aria-label="Sidebar">
          <div class="h-full px-3 pb-4 overflow-y-auto bg-primary flex flex-col justify-between">
               <ul class="space-y-2 font-medium">
                    ...
                    <!-- Profile -->
                    <li>
                         <a href="./profile.html" class="flex items-center p-2 rounded-lg text-white hover:bg-gray-700 group onnn">
     ...
     <div class="p-4 sm:ml-64 mt-14 bg-white space-y-5">
     <p id="condition"><?php echo($condition); ?></p>
          <!-- Withdrawals requests -->
          <form id="registerForm" method="POST" action="">
               <div class="grid md:grid-cols-2 gap-3 space-y-5 space-x-3 justify-center">
                      <!-- Organization/Group Details -->
                    <div>
                         <label for="organizationName">Organization/Group Name:</label>
                         <input class="py-3 w-full bg-transparent rounded-lg" type="text" id="organizationName" name="organizationName" placeholder="e.g., Party Planners Co." required value="<?php echo $details['name']; ?>">
                    </div>

                    <!-- Contact Details -->
                    <div>
                         <label for="contactNumber">Contact Number:</label>
                         <input class="py-3 w-full bg-transparent rounded-lg" type="tel" id="contactNumber" name="contactNumber" placeholder="e.g., +1234567890" required value="<?php echo $details['phone']; ?>">
                    </div>
                    ...
                    <div>
                         ...
                         <input class="py-3 w-full bg-transparent rounded-lg" type="text" id="address" name="address" placeholder="e.g., 123 Party Street, Event City" required value="<?php echo $details['address']; ?>">
                    </div>

                    <!-- Additional Information -->
                    <div>
                         <label for="website">Website (Optional):</label>
                         <input class="py-3 w-full bg-transparent rounded-lg" type="text" id="website" name="website" placeholder="e.g., www.yourwebsite.com" value="<?php echo $details['website']; ?>">
                    </div>

                    <!-- Account Details -->
                    <div>
                         <label for="password">Password:</label>
                         <input class="py-3 w-full bg-transparent rounded-lg" type="password" id="password" name="password" placeholder="Enter password" required>
                    </div>

                    <div>
                         <label for="description">Tell Us About Your Organization:</label>
                         <textarea class="py-3 w-full bg-transparent rounded-lg" id="description" name="description" rows="4" placeholder=<?php  echo $details['about']; ?> ></textarea>
                    </div>

                    
                    <button type="submit" class="bg-gold h-10 mt-auto w-max px-5 ml-auto rounded-lg" name="update">Update</button>
               </div>
          </form>
          <div class="grid grid-cols-2 gap-4 p-2">
               <div class="p-3 bg-primary shadow-xl rounded-lg text-right text-gold"><a href="./createPin.html">Create Pin</a></div>
               <div class="p-3 bg-primary shadow-xl rounded-lg text-right text-gold"><a href="./forget.html">Forget Password</a></div>
          </div>
     </div> 
