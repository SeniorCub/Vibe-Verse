...
$query = "DELETE FROM organizers WHERE `organizers`.`email` = ?";
...
if ($stmt = $conn->prepare($query)) {
    ...
    if ($stmt->execute()) {
        if ($stmt) {
            echo json_encode(['success' => true,"status" => 'success', "message" => "User Deleted", "url" => '/src/admin/organizers.html']);
        } else {
            echo json_encode(['success' => false,"status" => 'error', "message" => "User not Deleted. Please log in."]);
        }
    } else {
