...
$query = "SELECT * FROM `party` WHERE `organizerEmail` = ? && `id` = ?";
// Execute the query
// Execute the query
if ($stmt = $conn->prepare($query)) {
     $stmt->bind_param("ss", $sessionemail, $id); // Bind the email parameter
     ...
     if ($stmt->execute()) {
          ...
          echo json_encode(['success' => false, 'error' => 'Failed to execute query', 'url' => 'party.html']);
     }

     $stmt->close(); // Close the statement
} else {
     echo json_encode(['success' => false, 'error' => 'Failed to prepare statement: ' . $conn->error, 'url' => 'party.html']);
}
}
